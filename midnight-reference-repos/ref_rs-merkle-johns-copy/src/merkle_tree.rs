use crate::prelude::*;
use crate::{partial_tree::PartialTree, proof_tree::*, utils, utils::indices, Hasher, MerkleProof};
use std::boxed::Box;
...
impl<T: Hasher> MerkleTree<T> {
    ...
    /// Returns a Proof Tree (one layer below the root) of the given indices,
    /// which included the hash of the indices themselves.
    pub fn ordered_proof_tree(&self, leaf_indices: &[usize]) -> ProofNode<T::Hash> {
        let mut current_layer_indices = leaf_indices.to_vec();

        let mut proof_tree = Vec::new();
        // Set initial max of proof leaves similar to the leaves of this tree
        // This decreases as we go up to 1 layer below the root; a vec size of 2.
        proof_tree.resize_with(self.leaves_len(), || None);

        for (tree_layer_index, tree_layer) in self.layer_tuples().iter().enumerate() {
            // calculate the number of "leaves" on this layer
            let max_leaves = proof_tree.len();

            // Insert immediately the chosen indices, since current layer is at the bottom.
            if tree_layer_index == 0 {
                for curr_layer_index in &current_layer_indices {
                    if let Some((leaf_index, hash)) = tree_layer.get(*curr_layer_index) {
                        // construct as a leaf
                        let leaf = ProofNode::Leaf(hash.clone());
                        // insert to its position
                        proof_tree[*leaf_index] = Some(leaf);
                    }
                }
            }
            ...
            for index in helper_indices {
                if let Some(tuple) = tree_layer.get(index) {
                    // loop through the whole elements, starting at the sibling's index
                    let mut iter_index = tuple.0;

                    for _ in 0..max_leaves {
                        // if the position is empty, insert immediately
                        if let &None = &proof_tree[iter_index] {
                            let leaf = ProofNode::Leaf(tuple.1.clone());
                            proof_tree[iter_index] = Some(leaf);

                            break;
                        }

                        // current index/position has a proof node already; move to the next position
                        iter_index = (iter_index + 1) % max_leaves;
                    }
                }
            }

            proof_tree = temp_proof_layer::<T>(proof_tree);

            // move one layer up
            current_layer_indices = indices::parent_indices(&current_layer_indices);
        }
        if proof_tree.len() != 1 {
            panic!("not all layers are visited");
        }

        // flatten `Option`
        let res = proof_tree.pop().unwrap();
...
mod test {
    ...
    use crate::{
        proof_tree::{DfsIterator, ProofItem},
        utils::{self, collections::to_hex_string},
        Hasher,
    };
    ...
    use crate::{algorithms::Sha256 as Sha256Algo, MerkleTree};
    ...
    fn validators() -> Vec<[u8; 32]> {
        let data = [
            ALICE, BOB, CHARLIE, DAVE, EVE, FERDIE, GEORGE, HELEN, IAN, JASON,
        ];
        ...
        data.iter()
            ...
            .map(|(idx, d)| {
                ...
                hasher.update(bytes);
                let hash_bytes = <[u8; 32]>::from(hasher.finalize_fixed());

                let bytes_hex = utils::collections::to_hex_string(&hash_bytes);
    ...
    fn test_ordered_proof_tree() {
        ...
        let proof_serialize = println!("Proof: {proof:#?}\n\n");

        let mut proof_iterators = proof.as_dfs_iterator();

        while let Some(data) = proof_iterators.next() {
            match data {
                ProofItem::Leaf(leaf) => {
                    println!("{}", to_hex_string(&leaf));
                }
                ProofItem::Next => continue,
                ProofItem::Children(items) => {
                    let items: Vec<String> = items.iter().map(|item| to_hex_string(item)).collect();
                    println!("{items:#?}");
                }
            }
        }

        let og_proof = tree.proof(&chosen);
        let x = og_proof.proof_hashes_hex();
        println!("\n\n{x:#?}");
